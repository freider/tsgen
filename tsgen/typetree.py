...
class Object(AbstractNode):
    ...
    def ts_repr(self, ctx: CodeSnippetContext):
        ...
        if interface_name not in ctx:
            code = self.render_ts_interface(interface_name, ctx)
            ctx.add(interface_name, code)

        return interface_name

    def render_ts_interface(self, interface_name: str, ctx: CodeSnippetContext):
        ts_fields = []
...
type_registry = [Primitive, List, Object]
